...
const modeOptions = computed<TrainingModeOption[]>(() => [
  ...
  },
  {
    ...
    value: 'other-user',
    description:
      'Ответ открыт сразу, можно оценить пользователя и сохранить обратную связь в его карточку роста.',
    disabled: false,
  },
]);
...
const sessionFooterLabel = computed(() =>
  isActiveOtherUserMode.value
    ? 'Ответ открыт с первого шага. Оценки и обратная связь сохраняются в карточку роста выбранного пользователя.'
    : 'Навигация не меняет уже зафиксированные результаты.',
);
const exitDialogDescription = computed(() =>
  isActiveOtherUserMode.value
    ? 'Сохраняются уже выставленные оценки и введённая обратная связь для выбранного пользователя. Незаполненные карточки будут отброшены.'
...
async function persistSession(status: TrainingResultStatus) {
  ...
  try {
    ...
    pushToast(
      isActiveOtherUserMode.value
        ? status === 'COMPLETED'
          ? 'Результаты и обратная связь сохранены в карточку роста пользователя.'
          : 'Прогресс и обратная связь сохранены в карточку роста пользователя.'
        : status === 'COMPLETED'
    ...
    <UiPanel class="form-panel training-setup-panel" padding="default" variant="form">
      ...
      <section v-if="isOtherUserMode" class="form-section">
        <div class="training-selection-panel__copy">
          ...
          <small>
            Выбери активного пользователя. Его карточка роста получит оценки и итоговую обратную связь этой тренировки.
          </small>
        </div>
