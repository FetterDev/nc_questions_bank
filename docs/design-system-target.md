...
Статус: целевое описание для выравнивания редизайна. Это не implementation plan и не changelog. Документ фиксирует, к какому виду надо привести весь frontend после текущей первой волны системного редизайна.

## Цель

Привести приложение к единому современному enterprise-интерфейсу без смены пользовательских сценариев, API-контрактов и смыслов экранов. Визуальное направление: calm enterprise, плотный операционный продукт, нейтральная светлая тема, четкие границы, предсказуемая навигация, минимум декоративности.

Дизайн должен ощущаться как рабочий инструмент для банка вопросов, тренировок, интервью, модерации и аналитики. Не как landing page, не как маркетинговый SaaS hero, не как набор декоративных карточек.

## Принципы

1. **Плотность без тесноты.** Экран должен показывать больше полезных данных, но не за счет наложений, мелкого текста или случайных переносов.
2. **Одна система поверхностей.** Все панели, таблицы, формы, diff-карточки, пустые состояния и загрузка используют один набор токенов поверхностей.
3. **Роль действия очевидна.** Primary используется только для главного действия экрана или блока. Secondary и ghost не конкурируют с primary.
4. **Данные важнее декора.** Цвет нужен для состояния, роли, риска, уровня сложности или активного выбора. Не использовать цвет как фон ради настроения.
5. **Mobile first для плотных рабочих сценариев.** На мобильном экран не должен превращаться в бесконечную стопку из-за sidebar, пустых состояний или слишком крупных заголовков.
6. **Не менять UX по пути.** Перестановка сценариев, текстов, прав доступа и API не является частью дизайн-системы.

## Целевой фундамент

### Цвета

Текущая целевая база уже задана в `frontend/src/styles/foundation/tokens.css` и должна стать единственным источником визуальных решений:

- Page background: `--color-paper: #f6f7f9`.
- Основная поверхность: `--color-ivory: #ffffff`.
- Primary text: `--color-ink: #17212b`.
- Secondary text: `--color-ink-muted: #5f6f7f`.
- Teal для действий: `--color-cyan: #0f7f84`, hover `#0c686d`.
- Border: `--panel-border: #d8dee6`.
- Приглушенная поверхность таблиц/заголовков: `--table-header-bg: #eef2f6`.
- Field border: `--field-border: #cfd8e3`, focus `--field-border-focus: var(--color-cyan)`.
...
- Teal не использовать для больших декоративных фонов. Teal = действие, активный пункт, focus, selected.

### Типографика

Текущая база:

- UI/body/display: `IBM Plex Sans`.
